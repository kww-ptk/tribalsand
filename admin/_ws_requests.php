  ...
  <table class="data-table">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td><?= e(addon_label($a)) ?><?php if (($a['kind'] ?? '') === 'tour' && !empty($a['pax'])): ?> <span class="text-muted" style="font-size:12px">· <?= (int)$a['pax'] ?> pax</span><?php endif; ?><?php if (isset($a['price_amount']) && $a['price_amount'] !== null && (float)$a['price_amount'] > 0): ?> <span class="badge badge--grey"><?= e(format_price((float)$a['price_amount'])) ?></span><?php endif; ?><?php if (!empty($a['scheduled_for'])): ?> <span class="text-muted" style="font-size:12px">· <?= e(date('j M, H:i', strtotime((string)$a['scheduled_for']))) ?></span><?php endif; ?><?php if (!empty($a['requested_by_name'])): ?><div class="text-muted" style="font-size:12px">Requested by <?= e(guest_display_name(['passport_name'=>$a['requested_by_name']])) ?><?= !empty($a['requested_by_is_lead']) ? ' (lead)' : '' ?></div><?php endif; ?></td>
