...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['gallery_upload'])) {
    ...
    foreach ($_FILES['gallery_upload']['tmp_name'] as $i => $tmp) {
        ...
        $filename = seo_filename((string)($_FILES['gallery_upload']['name'][$i] ?? ''), 'room');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['gallery_action'])) {
    ...
    if ($act === 'add_library') {
        ...
        $added = 0;
        foreach ($keys as $k) {
            if (media_attach_to_gallery('room_images', 'room_id', $id, (string)$k, $room['name'] ?? '')) $added++;
        }
  ...
  <div class="card">
    ...
    <div class="card__body">
      ...
      <div style="margin:0 16px 16px;padding-top:14px;border-top:1px dashed var(--border)">
        ...
        <form method="POST" action="/admin/room-edit?id=<?= $id ?>" class="gallery-libform">
          <?= csrf_field() ?>
          <input type="hidden" name="gallery_action" value="add_library">
          <input type="hidden" id="roomLibKey" name="image_key" value="">
          <button type="button" class="btn-outline btn-sm" data-mp-open="roomLibKey"><?= admin_icon('image', 15) ?> Choose from library</button>
        </form>
      </div>
    </div>
  </div>
