...
<div class="card"><div class="card__body">
  ...
  <div id="amThread" class="am-thread"
    ...
    <div class="am-msg" data-mid="<?= (int)$m['id'] ?>" style="max-width:80%;<?= $am ? 'align-self:flex-end;background:#102F3A;color:#fff' : 'align-self:flex-start;background:#f3f4f6;color:#111' ?>;border-radius:12px;padding:9px 12px;font-size:14px;line-height:1.5">
      <?= e($m['body']) ?>
      <div style="font-size:11px;margin-top:4px;opacity:.7"><?= $am ? 'Staff' : 'Guest' ?> · <?= e(message_time_label($m['created_at'])) ?></div>
    </div>
    <?php endforeach; ?>
  </div>
  <form id="amForm" method="POST" action="/admin/booking.php">
    <?= csrf_field() ?>
