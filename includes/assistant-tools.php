...
function assistant_tool_definitions(bool $withRag = false, bool $withFacts = false, bool $withStaffOps = false): array {
    ...
    if ($withRag) {
        $tools[] = [
            ...
            'description' => 'Search the property, room, and activity DESCRIPTIONS for non-price information — what a villa or room is like, amenities and features, house rules, check-in/out and arrival instructions, the security deposit, Wi-Fi, area/location guides, getting here and transfers, booking terms (payments, deposits, cancellations, refunds), contact details, restaurants and dining, events and weddings, current offers, FAQs, nearby activities and tours, and sustainability initiatives. Use this for any "what/how/tell me about/is there…" question that is NOT about live availability or price. It returns short text passages; base your answer only on what comes back, and if nothing relevant is returned, say you do not have that information. Never use it to state prices or availability — use quote_stay / check_availability for those.',
            'input_schema' => [
                'type' => 'object',
                'properties' => [
...
function assistant_system_prompt(?array $venueScope, bool $withRag = false, string $audience = 'staff', bool $withFacts = false, bool $withStaffOps = false): string {
    ...
    $ragLine = $withRag
        ? "\n- You have NO built-in knowledge of Tribal Sand's properties, rooms, activities, policies, or surroundings — treat your own memory of them as empty. For ANY question about what a property or room is LIKE, its amenities or features, what there is to DO nearby, activities/tours, attractions, house rules, check-in/out, arrival, the security deposit, Wi-Fi, directions, transfers, booking terms, payment, cancellation, refunds, contact details, dining, events, weddings, offers, FAQs, or sustainability, you MUST call search_property_info FIRST and answer ONLY from what it returns. Do this EVERY time — even if a similar question was answered earlier in this conversation, and even if you believe you already know. If it returns nothing relevant, say you don't have that information; never fill the gap from general knowledge or plausible guesses.\n- Do NOT use list_properties to answer 'what is it like' — list_properties only maps a name to a slug. To describe a property or room, use search_property_info. Never use search_property_info for room prices or availability (it holds no live rates) — use the factual tools. Policy figures it returns (a deposit percentage, a cancellation window) are fine to quote as written. You may combine both: search_property_info for the description plus quote_stay/check_availability for the figures."
        : '';
