...
### Zuri restaurant sync — outbox/inbox, one owner per field
Two-way sync with Zuri's own system (`https://zuriwatamu.com/sync/v1`). **Runbook + stage flags: `docs/restaurant-sync.md`** — read it before touching any of this. Contract = Zuri's `docs/sync-contract.json` (never change a field on one side only). Tests: `tests/sync_*.php`, `tests/menu_sync_logic.php`, `tests/restaurant_*.php`.
- **We own** menu categories/items, tables, the ONE opening-hours record (`restaurant_hours`); **Zuri owns** sold-out (`item_availability` → `menu_items.is_sold_out`, NOT our "Hidden" `is_available`), customers; reservations are **creator-owned, status shared**. Never send a field we don't own — Zuri rejects the event `not_owner`.
- **ONE emit path:** every owned write runs in `menu_sync_tx()` and calls `menu_sync_emit()` (`includes/menu-sync.php`) — never a second copy of the version bump/loop guard. Only the synced venue (`SYNC_VENUE_SLUG`, default `zuri`) emits. Deletes are soft.
- **Inbound** is applied by `bin/sync-apply.php` (`includes/sync-apply.php`) inside `SyncContext::applying()` — anything written there must not echo back. A terminal reservation status never revives.
- **New bookings go through `reservation_book()`** (`includes/sync-reserve.php`), never `create_reservation()` directly from a form: for Zuri with `SYNC_RESERVATIONS` on it calls Zuri's `/reserve` (409 → alternatives; failure → flagged local request). Status changes go through `set_reservation_status()`, which emits only for bookings that exist on Zuri.
- Workers run from `docker/scheduler.sh` every 10 s behind a Postgres advisory lock; everything is inert until `SYNC_ENABLED` + the per-direction flags. Owner dashboard: Admin → Zuri sync.
...
- **Reply-To is the apex `reservations@tribalsand.com`.** `send_admin_reply()` (`includes/mail.php`) hard-sets `Reply-To` to `reservations@tribalsand.com` — the exact brand mailbox. That mailbox is on **M365** (the apex MX), which SES **cannot** receive, so automatic threading depends on an **M365 forward** rule: `reservations@tribalsand.com` → `reservations@inbound.tribalsand.com` (an address SES receives — the receipt rule is scoped to the whole `inbound.tribalsand.com` subdomain). The forward preserves the subject, so the `[TSR-<id>]` tag threads the reply back. **Without the forward, replies sit in the M365 mailbox for manual paste.** Deliberately **not** the `notify_email` setting (internal staff recipient; on dev it's a tester's inbox). The `[TSR-<id>]` subject tag still rides every reply, so automatic threading keeps working **when the reservations@ mailbox forwards to the inbound SES intake** (`INBOUND_MAIL_ADDRESS`); without that forward, replies sit in the M365 mailbox for staff to paste in (the old behaviour). So threading is now a mailbox-forwarding step, not the `INBOUND_MAIL_ADDRESS` env var — that var only names the SES receiving address for the webhook, no longer the Reply-To.
...
- Tests: `php tests/reports_logic.php` (pure aggregators + a ledger round-trip in a rolled-back transaction) and `tests/booking_import_logic.php` (amount parsing + ledger capture). **Apply `add_bookings_finance.sql` to prod RDS separately** (via `/admin/migrate.php`) — dev and prod databases are distinct.

### Travel-agent (trade) portal — isolated login, ONE price, requests never hold
External agents sign in at **`/agent/login.php`** (migration `add_travel_agents.sql`; accounts managed in Admin → Travel agents, `require_owner()`). Helpers in **`includes/agent.php`**; every read is pre-migration-safe (`agents_supported()`, `holds_agent_supported()`, `submissions_agent_supported()`, `holds_quoted_amount_supported()`). Test: `php tests/agent_portal_logic.php`.
...
- **A trade request NEVER places a hold — the owner's rule.** `agent/availability.php` shows live options (the same `ts_property_configurations()` the property pages use); `agent/request.php` → `agent_submit_request()` writes ONE `submissions` row (type `enquiry`; server-written `submissions.agent_id`; payload `agent_*` + `quoted_total` = net + HMAC `agent_sig`) and nothing else: no hold, no block, no availability sweep. Inventory is only ever blocked by reservations, who use **Convert to Hold** in `admin/submission-view.php`; that path calls **`agent_tag_converted_hold()`**, which links the hold (`holds.agent_id`, migration `add_holds_agent.sql`, after `add_travel_agents`) and freezes the agent's net **for the room actually booked** (re-quoted through `agent_stay_quote()`, so a swapped room or a combination's per-room hold is priced right) in `holds.quoted_amount` (`add_holds_quoted_amount.sql`). `bookings_sync_hold()` then books it as `source='agent'` + agency. **Apply both migrations on production** or converted trade holds book as website revenue at the rack rate. Never re-add automatic hold creation to the portal.
- **Contact of record = the agent.** `guest_email` on the request (and on the converted hold — staff keep it when converting) is the agent's login email, so the acknowledgement, the hold confirmation, cancellation/expiry mails, the manage link and admin replies all reach the trade partner; `guest_name` is the traveller; the traveller's own email/phone live only in the payload + message (never `guest_phone`). `send_guest_acknowledgement()` takes an optional `price` row — absent, the guest e-mail is unchanged.
- **"Your requests" is keyed on a server-written link, never the bare payload.** `agent_requests_filter()` selects `submissions.agent_id` or, for rows written before that column existed, payload `agent_id` **plus the HMAC `agent_sig`** (`agent_request_sig()`, keyed on `BOOKING_TOKEN_SECRET`); `agent_submission_agent_id()` applies the same rule when admin converts. A payload `agent_id` alone is client-posted — `api/trip-builder.php` stores its POST body wholesale, so anyone could plant one. Status per row: "Sent" until converted, then the hold's status.
- **Abuse guards (no Turnstile on an authenticated surface):** an identical re-send inside 30 s (`agent_recent_duplicate()` — same agent, dates and product/room set) reuses the earlier request instead of writing and mailing twice; `agent_request_throttled()` caps requests per 10 min (`AGENT_MAX_REQUESTS_PER_WINDOW`).
- **Known limits:** Maya Ilai is priced off the rate card and only the villa + studios can be requested — `agent_room_bookable()` refuses the six per-bedroom composite products (they own no units and are priced by the guest configurator), on the request page and in the writer; a combination is one request that reservations convert room by room; agents cannot cancel from the portal (they use the emailed manage link, like a guest).
...
- **pgvector, in the same Postgres.** `content_embeddings` (one row per chunk: `source`/`source_id`/`venue_id`/`title`/`chunk_index`/`chunk_text`/`content_hash`/`embedding vector(1536)`) with an HNSW cosine index. It is a **derived cache** — safe to TRUNCATE and rebuild from the live DB. Confirmed available on our AWS RDS PostgreSQL; `rag_supported()` = table exists **and** an embeddings key is set, so a deploy missing either just omits the descriptive layer (NFR4).
...
- **Reindex is a CLI, idempotent.** `bin/reindex-content.php` gathers editable prose (venue about/stay copy, room descriptions + features + FAQs, tours, sustainability, the arrival/check-in/deposit settings, current offers **without their price badge**, and the public pages listed in **`rag_site_pages()`** — terms, contact, events, dining, activities, area guides), chunks it (`rag_chunk`, ~1200 chars, paragraph-aware), embeds **only chunks whose `content_hash` changed**, and prunes removed docs/chunks. Runs **daily** from `docker/scheduler.sh` (Job 7), so copy edits reach the assistant within a day; run it by hand for an immediate refresh (`--dry-run` to preview). On-save reindex is still a follow-up. Local Windows dev needs the same CA-bundle `-d curl.cainfo=…` flag as the chat call.
- **Website pages are read RENDERED**, fetched from `site_url($path)` (`rag_fetch_page()`, `function_exists`-guarded for tests) and reduced by `rag_html_to_text()`, which drops nav/drawer/footer/cookie banner, scripts, forms and modals and keeps block breaks for chunking. So admin page-content edits are what the AI learns. **To teach it a new page, add the clean path + a descriptive title to `rag_site_pages()`.** A page that fails to load is marked `keep_existing` and its previous chunks are kept rather than pruned — one bad fetch must never make the assistant forget a page. The AI can only repeat what the pages say: the site has **no room cancellation policy** today (`tc.php` is restaurant-focused), so that answer stays vague until the policy is written somewhere indexed.
...
### AI gaps — where the guest concierge fails (read-only)
**Admin → AI gaps** (`admin/ai-gaps.php`, `require_manager()`) lists guest questions the concierge answered badly, read from `concierge_log` — **no migration, no writes**. Logic in **`includes/ai-gaps.php`**; pre-migration-safe via `concierge_log_supported()`. Test: `php tests/ai_gaps_logic.php`.
- **Four reasons, one per turn, most serious first:** `failed` (`ok = false`), `unsure` (the answer admits it doesn't know or hands off — "I don't have", "please contact"…), `no_lookup` (a price/date/availability question whose answer **states a price** with **zero tool calls** — the figure can't have come from our rates; a clarifying question back with no lookup is correct and NOT flagged, see `ai_gap_answer_has_price()`), `repeat` (the same session asked a near-identical question later — the EARLIER turn is flagged, since that answer didn't land).
- **Classification is in PHP**, not SQL (text rules), over the newest `AI_GAPS_MAX_ROWS` turns of the window so a busy month can't slow the page. Topics are keyword buckets checked in order (first match wins — specific topics like transfers sit above generic price). **Keywords match whole words (plurals allowed); a trailing `*` makes a prefix** (`activit*`, `book*`). Plain substring matching misfired ("cat" in "cater", "pet" in "carpet") — don't revert it.
- **Never selects `client_ip`** — the page has no reason to show who asked.
- The page's "how to fill a gap" copy points general facts to AI settings → Knowledge, property detail to the property copy (picked up by the daily reindex), and priced/dated facts to a new read-only lookup tool — never a hardcoded number in the prompt.
...
- **The brain is `ts_property_configurations($venue, $ci, $co, $guests, ?$rooms, $always_combos=false)`** in `includes/db.php` (beside `ts_search_availability`). Returns `singles` (individual rooms whose one unit fits: `capacity >= guests`, free), `entire` (whole-property option when free), `combos` (ranked multi-room suggestions), and `max_capacity` (largest party the window can host). By default **`combos` are computed ONLY when no single room fits** the party — but they ARE offered even when the whole place is free (a combo can be cheaper than the buyout), so both show. **`$always_combos=true`** (passed by `api/property-availability.php`, the property-page widget) ALSO suggests combos when a single room fits (e.g. two doubles for a party of 4, cheaper than one quad); it then filters to genuine MULTI-room combos via `ts_rank_combos(..., $minUnits=2)` so a 1-unit "combo" never just duplicates a listed single. Search + the AI keep the default (singles-first) behavior.
...
### Availability-first property pages (Phase 3)
Multi-room property pages (`maya-kobe.php`, `zuri.php`, `maya_ilai.php`) **no longer preselect a room**. Their sidebar is `includes/property-availability-widget.php` (set `$pa_venue_slug` before the include); entire-place-only pages (`my-amani`, `enkare-bofa`, `sandbox`) keep the per-room `includes/booking-widget.php`.
- **Flow:** dates (shared `.dp-btn` datepicker) + guests steppers → "Check availability" → `GET /api/property-availability.php?venue=&check_in=&check_out=&adults=&children=` (JSON; mirrors `api/check-availability.php` guards — `rates_window_ymd()` 422, published venues only, CORS; returns `ts_property_configurations()` for the one venue — the ONE pricing path, no new nightly loop). Options render **inline**.
- **Select a single room / the whole property** → opens the existing booking modal (`tsOpenBookingModal`, prefilled dates + guests) = the normal 24h-hold / enquiry flow, unchanged. The widget **emits `includes/booking-modal.php` itself** (once, via `$GLOBALS['__pa_modal_done']`) so the modal is present; `booking-modal.js`/`booking-widget.js` are already loaded by `$page_rooms_rates`. Those pages set `$page_rooms_rates = true`, and `rooms-and-rates.php`'s `rrBook()` now opens that same modal.
- **Combo booking model = v1 (combo → prefilled enquiry).** A combination's "Request these rooms" opens a small enquiry modal posting to `api/submit-contact.php` (Turnstile fail-closed + IP rate-limit on the endpoint; honeypot in the form) with the room list / dates / guests composed into the message. **A true atomic multi-room hold is the v2 follow-up** (multiple holds + one confirmation + `bookings_sync_hold` per room) — not built; see the plan §6.4. Prefill from `?checkin&checkout&adults&children` (search's "Request these rooms" links here) auto-runs the check. No-JS: a link to `/search`.

### AI drafts enquiry replies (Phase 4)
From `admin/submission-view.php`, **"Draft options with AI"** (shown only when `ai_assistant_supported()`) posts the submission id to `api/assistant-draft.php` → runs `chat_with_tools()` with the **staff** persona and drops a warm, availability+price reply into the reply box to **edit and send** (never auto-sends; staff use the existing `add_note` + `send_admin_reply()` path). Same READ-ONLY tools / ONE pricing path as the assistant. Guards mirror `api/assistant.php`: session-authed, `require_frontdesk()` audience, CSRF token in the JSON body, `admin_venue_ids()` scope, plus a `submission_in_scope()` re-check. `assistant_build_draft_brief($sub)` (in `includes/assistant-tools.php`) is pure/testable and folds in the owner's `ai_draft_instructions` (Phase 5).

### Point of Sale — outlets, one sale path, room charge onto the bill
Till at **`/pos/`** (standalone, tablet-first; UI ref `docs/pos/pos-prototype.html`), admin under **Point of Sale**. Plan: `docs/pos/POS-PLAN.md`. Migrations: `add_pos.sql` → `add_pos_job_types.sql` → `add_pos_v2.sql` (VAT, tips, room-charge properties, FX, signatures, per-delivery consignment terms; every v2 path is guarded by `pos_v2_supported()` and degrades to v1 behaviour). Logic **`includes/pos.php`**; till auth **`includes/pos-auth.php`**; the light guards `pos_supported()` / `pos_bill_link_supported()` / `pos_is_seller()` live in **`includes/pos-support.php`** (the admin sidebar + Bill tab load only that). Seed `db/seeds/seed_pos.php` (non-destructive, `--dry-run`). Test: `php tests/pos_logic.php` (pure always; DB block in one rolled-back transaction, SKIPs with no DB).
- **`pos_complete_sale()` is the ONE write path; `pos_void_sale()` the only reversal.** The till (`js/pos.js`) posts item ids + qty (+ `open_price` for an unpriced item) — **every line is re-priced server-side** (`pos_resolve_line()`: item price → linked tour's `price_amount` → open price). Idempotent on `client_uuid` (UNIQUE; a racing duplicate's 23505 returns the existing sale); the till keeps the SAME uuid across retries of one sale and mints a new one only after success or a 422 refusal. Sales are immutable; corrections are a void (owner/manager of the outlet, reason required) that restocks and deletes the linked bill line.
- **Room charge = ONE `bill_items` row with `pos_sale_id`** (unique partial index), `guest_id` = the chosen adult, label `"<Outlet> (<property>): <items> — <note> (POS-…)"` — so it shows on the admin Bill tab, bill print and guest portal with no new display code. Eligibility is pure `pos_room_charge_eligible($hold, $today, $outlet)`: confirmed or staff-typed pending (no TTL), `check_in ≤ today ≤ check_out` (Nairobi), outlet room charge on, and the guest's property in `pos_outlet_charge_venues` — **no rows = every property** (owner's rule: Tribal Dunes/Maya Ilai/Off Duty guests shop across the complex). The till flags a guest from another property ("guest of Maya Ilai").
- **Outlets sell in KES (`POS_DEFAULT_CURRENCY`); room bills are in `site_currency` (USD).** A room charge from another currency is converted at the site FX table (`pos_fx_rate()` = units of sale currency per 1 bill unit; `pos_fx_convert()` exact to the cent — NOT `convert_price()`, which rounds to display units). The sale keeps `bill_currency`/`bill_amount`/`fx_rate`; the bill line holds the converted amount and its label carries "KES 3,080 @ 129 KES/USD". No rate → room charge refused, never guessed.
- **Totals (`pos_cart_totals()`, integer cents, mirrored in `js/pos.js`):** subtotal → service % on it → VAT on (subtotal+service): **inclusive** = `base×v/(100+v)` already in the price (default, Kenyan shelf prices), **exclusive** = added on top → tip last (`pos_tip_cents()`: % of the pre-tip total or an amount, ≤100%), never carrying service or VAT. Per-outlet: `vat_pct`, `vat_inclusive`, `tips_enabled`; the sale snapshots `service_pct`/`vat_pct`/`vat_inclusive`/`vat_amount`/`tip_amount`.
- **Guest signature for room charges** (`require_signature`, default on): the till reuses `js/signature-pad.js` (same as check-in); `pos_valid_signature()` checks a real PNG data URL ≤250 KB; stored in `pos_sale_signatures` (private — only the authenticated receipt and admin sale page render it). The server refuses an unsigned room charge regardless of the client. **`admin/booking.php` `bill_del` refuses POS rows server-side** and `_ws_bill.php` shows a POS badge instead of the trash icon — keep both.
- **Stock is a ledger:** `pos_stock_moves` is the truth, `pos_items.stock_qty` its cached total, both written by `pos_stock_move()` under `SELECT … FOR UPDATE` (items locked in id order). Oversell refused before any write unless the item allows negative. Admin changes stock only via receive / count (`pos_stock_receive()`, `pos_stock_adjust()` — count needs a reason). `pos_tx()` nests with SAVEPOINTs so a refused sale inside an outer transaction rolls back only its own writes. Money is integer-cents; one currency per outlet; reports are always per currency.
- **Access.** POS users are `admin_users`. `pos_user_outlet_ids($user)`: owner → all active; manager → their venues' outlets + assigned; others → `pos_outlet_staff` only; cross-sell via `pos_outlet_links` (sale line keeps `owning_outlet_id`). It takes the user ROW, so PIN and admin sessions resolve identically. Every endpoint re-checks outlet access (`pos_ctx_outlet()`); the client's outlet id is a request, not a fact.
- **Two ways into `/pos/`** (`pos_current()`): a browser **without** a terminal cookie runs as the signed-in admin; a **registered tablet always runs in terminal mode** — lock screen → tap name → PIN — even when an admin session exists, and a revoked/unknown cookie never falls back to admin mode. Terminals are registered ON the tablet (`/pos/register.php`, owner/manager): token in an httpOnly cookie, sha256 in `pos_terminals`; registering **ends the admin session on that device**. A PIN session sets `pos_user_id`/`pos_terminal_id`, **never `admin_id`** (it opens no admin page); outlets = user ∩ terminal; 12 h cap; idle lock (`pos_idle_lock_seconds` setting, default 120 s) client-side with a 60 s server grace + a 45 s heartbeat.
- **PINs:** 4–6 digits, `password_hash`, trivial ones refused (`pos_pin_problem()`), 5 wrong per user / 20 per IP in 15 min → locked (the correct PIN is refused too); setting a PIN clears the lockout. PIN attempts share `login_attempts` keyed `pos:<id>`, so **`is_rate_limited()` excludes `pos:%` rows** — otherwise a few wrong PINs at a till would lock every admin login from the property's IP. Set in Admin → POS → Staff PINs (`admin/pos-pins.php`: own PIN; owner any; manager their outlets' staff).
- **Job types** `shop` / `spa` / `kite` (`add_pos_job_types.sql`): `job_is_pos()`, home `/pos/`, no guest messaging (`require_frontdesk()` bounces them). `staff_job_types()` in `admin/staff.php` offers them only once `pos_jobs_supported()` (a `pg_constraint` lookup).
- **Consignment:** terms are chosen **when a delivery is received** (Stock page: ours / consignment % / fixed per item → `pos_consign_terms()` + `pos_stock_receive(..., $terms)`), which sets the item's `consignor_id` + `consign_pct` or `consignor_cost` and records them on the `pos_stock_moves` row. Precedence at sale: fixed cost → item `consign_pct` → supplier default %. A line snapshots `consignor_id` + the terms used; `pos_consignor_owed()` / `pos_consignment_statement()` compute payouts from that snapshot, never the current terms. Payouts: `pos_consignor_payouts` (Suppliers page → Statement → Paid). Activities are **linked** (`pos_items.tour_id`, price NULL), never copied; spa services seed **inactive + unpriced**.
- **Admin pages:** `pos-sales.php` (history + per-currency totals + CSV + sale detail/void + Z-report `?view=z`), `pos-outlets.php` (**owner-only**), `pos-items.php`, `pos-stock.php`, `pos-consignors.php`, `pos-terminals.php`, `pos-pins.php` — all but outlets are `require_manager()` scoped by `pos_manageable_outlet_ids()` (includes closed outlets); every mutation re-checks the row's outlet.
- **Responsive:** the till has phone (≤560: 2-col grid, bottom-sheet order panel, bottom-sheet modals), tablet (≤900 sheet, ≤1100 icon rail) and desktop layouts; admin POS pages collapse to one column with `minmax(0,1fr)` (a plain `1fr` track refuses to shrink below a wide child and scrolls the whole page sideways on phones). Check new POS UI at 375 and 768 for horizontal overflow.
- **Prod:** run `add_pos.sql`, `add_pos_job_types.sql`, then `add_pos_v2.sql` via `/admin/migrate.php`, then `seed_pos.php`. Nothing shows until then (nav hidden by `pos_supported()`). Not built (plan P7): email receipts, M-Pesa STK push, ESC/POS printer, offline queue, POS revenue on `admin/reports.php`, `pos.tribalsand.com` alias (URLs are root-relative, so it's a host alias).
...
| `includes/pos.php` · `includes/pos-support.php` | POS — pure totals/pricing/eligibility, permissions, stock ledger, `pos_complete_sale()` / `pos_void_sale()`, till payloads, Z-report, consignment statement |
| `includes/pos-auth.php` | Till access — admin vs terminal mode (`pos_current()`), terminal cookie, PIN login + lockout, JSON endpoint guard |
| `pos/index.php` · `js/pos.js` · `css/pos.css` | The till: lock screen, outlets, cart, in-house/walk-in customer, payments, review, receipt, history, void |
| `pos/register.php` · `pos/receipt.php` | Register a tablet as a till (ends the admin session on it) · printable 80 mm receipt |
| `api/pos/*.php` | Till JSON: `catalog`, `inhouse`, `customers`, `sale`, `sales`, `void`, `session` (heartbeat / PIN / lock) |
| `admin/pos-sales.php` | POS sales history, sale detail + void, CSV, daily Z-report (per currency, manager-scoped) |
| `admin/pos-outlets.php` | POS outlets (owner-only) — property, currency, service %, room charge, staff, cross-sell links, categories |
| `admin/pos-items.php` · `admin/pos-stock.php` · `admin/pos-consignors.php` | Catalogue (activities linked, photos, consignment), stock receive/count + ledger, suppliers + statement/payouts |
| `admin/pos-terminals.php` · `admin/pos-pins.php` | Registered till tablets (rename / outlets / revoke) · staff till PINs |
...
| `includes/assistant-tools.php` | Read-only assistant tool layer — wrappers, schemas, composed system prompt (hard rules + owner-editable persona/knowledge), `assistant_build_draft_brief()` |
...
| `api/assistant-draft.php` | Enquiry-reply drafter (Phase 4) — `require_frontdesk`, CSRF-in-body, scoped; drafts a ready-to-send reply into the thread (never sends) |
...
| `admin/ai-settings.php` | Owner-editable AI tone/knowledge/draft-style (Phase 5) — `settings` KV, owner-only |
| `api/property-availability.php` | Single-venue configurations (Phase 3) — JSON singles/entire/combos/max_capacity for the availability-first page widget |
| `includes/property-availability-widget.php` | Availability-first property sidebar (Phase 3) — dates+guests → inline options; Select→booking modal, combo→enquiry; bundles the booking + combo-enquiry modals |
...
| `includes/ai-gaps.php` · `admin/ai-gaps.php` | AI gaps — pure classifier of weak concierge answers + the read-only owner/manager list |
