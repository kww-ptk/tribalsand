...
- **"Your requests" is keyed on a server-written link, never the bare payload.** `agent_requests_filter()` selects `submissions.agent_id` (same migration) or, for rows written before that column existed, payload `agent_id` **plus the HMAC `agent_sig`** (`agent_request_sig()`, keyed on `BOOKING_TOKEN_SECRET`). A payload `agent_id` alone is client-posted — `api/trip-builder.php` stores its POST body wholesale, so anyone could plant one — and must never decide whose request a row is.
- **Abuse guards (no Turnstile on an authenticated surface):** an identical re-send inside 30 s (`agent_recent_duplicate()` — same agent, dates and product/room set) reuses the earlier request instead of writing a second hold and mailing twice; `agent_request_throttled()` caps requests per 10 min and pending holds per agent (`AGENT_MAX_*`). The stale-hold sweep runs ONCE **before** the request transaction (`expire_stale_holds()` then `find_available_unit_internal(…, false)`): inside it, a 409/500 rollback would resurrect holds whose expiry e-mails had already gone out.
- **Known limits:** Maya Ilai is priced off the rate card and only the villa + studios are bookable — `agent_room_bookable()` refuses the six per-bedroom composite products (they own no units and are priced by the guest configurator), on the request page and in the writer, so they cannot be booked by URL either; agents cannot cancel from the portal (they use the emailed manage link, like a guest).
