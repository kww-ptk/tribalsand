...
- **Room charge = ONE `bill_items` row with `pos_sale_id`** (unique partial index), `guest_id` = the chosen adult, label ends with the `POS-…` reference — so it shows on the admin Bill tab, bill print and guest portal with no new display code. Eligibility is pure `pos_room_charge_eligible()`: confirmed or staff-typed pending (no TTL), `check_in ≤ today ≤ check_out` (Nairobi), outlet venue = hold venue (a venue-less outlet takes any property), **outlet currency = `site_currency`** (never converted), outlet room charge on. **`admin/booking.php` `bill_del` refuses POS rows server-side** and `_ws_bill.php` shows a POS badge instead of the trash icon — keep both.
...
- **Consignment:** a line snapshots `consignor_id` + fixed `consignor_cost` (wins) or the supplier's `commission_pct`; `pos_consignor_owed()` / `pos_consignment_statement()` compute payouts from that snapshot, never the current terms. Payouts: `pos_consignor_payouts` (Suppliers page → Statement → Paid). Activities are **linked** (`pos_items.tour_id`, price NULL), never copied; spa services seed **inactive + unpriced**.
