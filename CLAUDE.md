...
- **Reply-To is always the reservations mailbox.** `send_admin_reply()` (`includes/mail.php`) hard-sets `Reply-To` to `reservations@tribalsand.com` — the recognisable brand address, **never** the `reply@inbound.*` SES subdomain and deliberately **not** the `notify_email` setting (that's the internal staff-notification recipient and may point elsewhere — on dev it's a tester's inbox). The `[TSR-<id>]` subject tag still rides every reply, so automatic threading keeps working **when the reservations@ mailbox forwards to the inbound SES intake** (`INBOUND_MAIL_ADDRESS`); without that forward, replies sit in the M365 mailbox for staff to paste in (the old behaviour). So threading is now a mailbox-forwarding step, not the `INBOUND_MAIL_ADDRESS` env var — that var only names the SES receiving address for the webhook, no longer the Reply-To.
