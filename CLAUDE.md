...
- **Reply-To is a received "reservations@" subdomain address.** `send_admin_reply()` (`includes/mail.php`) hard-sets `Reply-To` to `reservations@inbound.tribalsand.com` — reads as the reservations brand, and because `inbound.tribalsand.com` is a subdomain **SES receives on**, a guest's reply lands at SES → the `[TSR-<id>]` tag threads it back automatically with **no M365 forward needed**. It is deliberately **not** the apex `reservations@tribalsand.com` (that mailbox is on M365; SES can't receive it) nor the `notify_email` setting (internal staff recipient; on dev it's a tester's inbox). The address **must be accepted by the SES receipt rule** (add it as a recipient, or scope the rule to the whole `inbound.tribalsand.com` domain). The `[TSR-<id>]` subject tag still rides every reply, so automatic threading keeps working **when the reservations@ mailbox forwards to the inbound SES intake** (`INBOUND_MAIL_ADDRESS`); without that forward, replies sit in the M365 mailbox for staff to paste in (the old behaviour). So threading is now a mailbox-forwarding step, not the `INBOUND_MAIL_ADDRESS` env var — that var only names the SES receiving address for the webhook, no longer the Reply-To.
