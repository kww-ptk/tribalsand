...
<?php $rr_venue_slug = 'zuri'; include __DIR__ . '/includes/availability-bar.php'; ?>
      ...
      <div class="book-body">
        ...
        <div class="book-enquiry" id="enqWrap">
          ...
          <div class="enq-step" id="enqStep1">
            ...
            <div class="book-field">
              <label class="book-lbl" for="enqRooms">Suites</label>
              <select class="book-inp" id="enqRooms">
                <option value="">—</option>
                <option>1 Suite</option><option>2 Suites</option><option>3 Suites</option>
                <option>4 Suites</option><option>5 Suites</option><option>Buyout (6)</option>
              </select>
            </div>
...
(function(){
  ...
  document.getElementById('btnEnquire').addEventListener('click', function(){
    ...
    fetch('/ghl-submit', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({
      ...
      trip:    { prop: 'Zuri', arrival: arrival, departure: departure, adults: adults, children: children, rooms: rooms },
      message: message,
      tags:    ['website-enquiry', 'zuri-enquiry'],
      opportunity: { source: 'Website - Zuri' },
      note:    noteLines.join('\n'),
      ref:     'WEB-' + Date.now()
    })})
