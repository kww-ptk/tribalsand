...
- **`updated_at` does not update itself.** There is no trigger and no rule on `restaurant_reservations` — verified against the live schema. Every `UPDATE` in Tasks 9 and 10 must write `updated_at = NOW()` explicitly, or the column silently stays equal to `created_at` forever and the admin shows wrong "last changed" times with no error to warn anyone. The codebase has no update-trigger convention elsewhere, so adding one only for this table would be the inconsistent choice.
...
> **Note — this task also extends `restaurant_validate()`.** `notes` and `phone` are stored by
> this endpoint but were not validated by Task 2, so an array in either emitted a warning into
> the JSON response body, an unbounded `notes` let an anonymous caller write megabytes, and a
> 51-character phone (an ordinary thing to type) exceeded `VARCHAR(50)` and returned a 500 with
> no `errors.phone` for the form to show. A null byte in any field silently truncated the stored
> value under emulated prepares. All four rules live in `restaurant_validate()` rather than
> inline here, because Task 10's admin edit path is a second writer and must inherit them.
